...
(function($) {
	$.fn.lightBox = function(){
		...
		function resizeX(oImage){
			...
			iMaxX = iWinX - (getFullWidth($('#lightBox .containerg')) - iImgX);
		...
		function resizeY(oImage){
			...
			iMaxY = iWinY - (getFullHeight($('#lightBox .containerg')) - iImgY);
			...
			$('#lightBox .containerg').css({
				'top' : ($(window).scrollTop() + 30 + ((iWinY - getFullHeight($('#lightBox .containerg'))) * 0.5)) + 'px'
			});
		}
		...
		function bindEvents(oImages, oImage){
			...
			if(typeof(oImage) !== 'undefined'){
				...
				$(document).on('click tap', '.containerg', function(event){
					event.stopPropagation();/* make sure clicking on .containerg doesn't trigger click on #overlay */
				});
		...
		function init(){
			...
			if(!document.getElementById('lightBox')){
				$('body').append('<div id="overlay"></div><div id="lightBox"><span class="containerg"><span class="image"><a href="javascript:void(0);" id="closeBtn"></a><a href="javascript:void(0);" id="nextImage" class="navigatory"></a><a href="javascript:void(0);" id="prevImage" class="navigatory"></a></span><span class="data"></span></span></div>');
				oImage = new Image();
